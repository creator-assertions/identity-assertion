...
* `pad1` and `pad2` are zero-filled (binary `0x00` values) byte strings used to fill space
...
identity = {
  ...
  "pad1": bstr, ; zero-filled (binary `0x00` values) byte string used for filling up space
  ? "pad2": bstr, ; optional zero-filled (binary `0x00` values) byte string used for filling up space
}
----
