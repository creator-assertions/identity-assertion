...
The endpoint address is obtained via the `endpoints.user_options` member of the <<Configuration and discovery>>_ response.
...
In the event the service is unable to respond successfully to any of the requests described here, the service SHOULD respond with an appropriate HTTP status code (4xx or 5xx). This specification does not specify the response body for error cases.
