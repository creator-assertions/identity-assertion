...
* `claim_certificate_hash` (optional) is the hash of the leaf-certificate of the claim signing key.  ID-assertion-generators can set this field to mitigate the threat of the authorized Claim Signature being stripped and replaced. The hash is calculated over the leaf certificate of the Claim Signing key using the same representation as used in the signature structure (i.e., DER for X.509). 
