...
sequenceDiagram
  ...
  note right of ICA: Verifies presentation
  note right of ICA: Remembers information
....
...
The _<<_identity_claims_aggregator,identity claims aggregator>>_ then verifies this presentation and retains the information contained therein for future use.
...
sequenceDiagram
  ...
  note right of A: Creates asset

  A ->> C2PA: Prepare C2PA Manifest
  C2PA ->> C2PA: Starts generating C2PA Manifest
  C2PA ->> ICA: Sign signer_payload for Alice
  ICA ->> C2PA: Signature
...
1. When Alice is ready to render her _<<C2PA asset>>,_ she signals to the authoring tool that she wants it to include a _<<_c2pa_manifest,C2PA Manifest>>_ with an *<<_identity_assertion,identity assertion>>* describing her role in producing the asset.

2. The C2PA implementation begins the process of creating the _<<_c2pa_manifest,C2PA Manifest>>_ and *<<_identity_assertion,identity assertion>>* as described in xref:_creating_the_identity_assertion[xrefstyle=full].

3. The C2PA implementation sends a request to the _<<_identity_claims_aggregator,identity claims aggregator>>_ containing the `signer_payload` for this _<<C2PA asset>>_ and a user token that uniquely identifies Alice within that system.

4. Assuming the token is valid, the _<<_identity_claims_aggregator,identity claims aggregator>>_ returns a _<<W3C verifiable credential>>_ known as an _<<_identity_claims_aggregation,identity claims aggregatyion>>_ and uses the _<<_identity_claims_aggregator,identity claims aggregator’s>>_ signature to bind the _<<C2PA asset>>_ description (`signer_payload`) to any identity signals remembered from Step 1.

5. The C2PA implementation includes this credential as the `signature` value for the *<<_identity_assertion,identity assertion>>.*

6. The C2PA implementation finalizes the _<<_c2pa_manifest,C2PA Manifest>>,_ including adding its own signature as _<<_c2pa_claim_generator,C2PA claim generator>>._

7. This finalized _<<C2PA asset>>_ is then made available to Alice who can now distribute it as she wishes.
