...
[NOTE]
.Example of trust signal scope with one identity assertion
====
The diagram that follows, with one _<<C2PA Manifest>>_ containing one *<<_identity_assertion,identity assertion>>,* is intended to illustrate the most basic example of the two trust signals available and how they interesect:
...
The yellow-colored circle demonstrates the limit of *<<_identity_assertion,identity assertion 1’s>>* trust signal.
...
[NOTE]
.Example of trust signal scope with two non-overlapping identity assertions
====
The diagram that follows, with one _<<C2PA Manifest>>_ extends the previous example by showing how two containing two *<<_identity_assertion,identity assertions>>* could be used to describe distinct assertions attested to by two distinct _<<_actor,actors>>._
...
The blue-colored circle demonstrates the limit of *<<_identity_assertion,identity assertion 2’s>>* trust signal.
