...
NOTE: TO DO: Revise above example to reflect more common scenario where issuer ≠ named actor.
...
NOTE: TO DO: Consider removing this section. I don’t think it adds any new requirements over the existing W3C VC requirements.
...
NOTE: TO DO: Consider merging this down into `credentialSubject` and making verifications its own property.
...
NOTE: TO DO: Define what we mean by "proof of authenticity" here.
...
NOTE: TO DO: Consider changing the identity type names to `snake_case` for consistency with the rest of the spec. Consider using `cawg.` prefix to allow non-CAWG identity types to be used here.
...
NOTE: TO DO: Specify Base-64 encoding options.

NOTE: TO DO: Consider name of this property. I'm not sure `boundTo` conveys the meaning fully.

NOTE: TO DO: Decide whether to preserve `snake_case` names or translate to `camelCase` for `signer_payload` content. Current example uses `camelCase`.
...
NOTE: TO DO: Track changes to draft schema and ensure example is correctly updated.
...
NOTE: TO DO: Update schema URL once it is finalized.

NOTE: TO DO: Add text explaining the use of `credentialSchema` property as per the example below.
...
NOTE: TO DO: Merge with above section? This explains the use of `credentialSchema` to answer my earlier TO DO item.
...
NOTE: TO DO: Write section on proofs.

==== Validating an identity assertion with verifiable credentials

NOTE: TO DO: Write section on validating.
