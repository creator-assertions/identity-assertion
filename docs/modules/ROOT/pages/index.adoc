...
If the `alg` header contains any other value or is not present, the validator MUST stop validation at this point and issue the failure code `cawg.ica.invalid_alg`.

The validator SHALL inspect the `COSE_Sign1` protected header `content type` to determine the content type of the enclosed credential. The `content type` header MUST be the exact value `application/vc`. If it is not, the validator MUST stop validation at this point and issue the failure code `cawg.ica.invalid_content_type`.
...
NOTE: TO DO: Read W3C VC verification: https://www.w3.org/TR/vc-data-model-2.0/#verification (§7.1) and see if additional items need to be added here. Also figure out how to reconcile W3C "problem details" (§7.2) with C2PA validation status.

====== Obtain the credential issuer’s public key

The validator SHALL obtain the identity of the identity claims aggregator by inspecting the `issuer` field of the verifiable credential. The identity SHOULD be expressed as a DID (link:https://www.w3.org/TR/did-1.0/[decentralized identifier]), either as `issuer` itself or `issuer.id`. If no DID is located at either location, the validator MUST stop validation at this point and issue the failure code `cawg.ica.invalid_issuer`.

The validator SHALL resolve the DID document as described in link:++https://www.w3.org/TR/did-1.0/#did-resolution++[Section 7.1, “DID resolution,”] of the DID specification. If the DID uses a DID method that is unsupported by the validator, the validator MUST stop validation at this point and issue the failure code `cawg.ica.did_unsupported_method`. If the DID can not be resolved, the validator MUST stop validation at this point and issue the failure code `cawg.ica.did_unavailable`.

The validator SHALL locate within the DID document the `assertionMethod` verification method as described in link:++https://www.w3.org/TR/did-1.0/#assertion++[Section 5.3.2, “Assertion,”] of the DID specification. This verification method SHALL contain public key material corresponding to the stated issuer. If the public key material can not be located, the validator MUST stop validation at this point and issue the failure code `cawg.ica.missing_public_key`.

The validator SHALL verify that the `type` code for this public key is on the supported list of encryption algorithms.

NOTE: TO DO: Add list of supported encryption types and the corresponding type codes.

If the `type` code is unsupported, the validator MUST stop validation at this point and issue the failure code `cawg.ica.unsupported_alg`.

NOTE: TO DO: Add verification of issuer DID against validator’s trust list(s).

====== Verify the COSE signature

The validator SHALL verify the signature using the public key material just identified and the unsecured verifiable credential as payload as described by link:++https://www.rfc-editor.org/rfc/rfc9052#section-4.4++[Section 4.4, “Signing and verification process,”] of RFC 9052. If the signature does not match, the validator MUST stop validation at this point and issue the failure code `cawg.ica.signature_mismatch`.
...
The validator SHALL take the content of `signer_payload` in the *<<_identity_assertion,identity assertion>>* and perform the transformations from CBOR to JSON as described in xref:_binding_to_c2pa_asset[xrefstyle=full]. The validator SHALL then compare the transformed `signer_payload` data structure to the `c2paAsset` field contained within the verifiable credential’s `credentialSubject` field. If the data structures do not match, the validator MUST stop validation at this point and issue the failure code `cawg.ica.signer_payload.mismatch`.

===== Verify verified identities

The validator SHALL inspect the contents of the `verifiedIdentities` field contained within the verifiable credential’s `credentialSubject` field. If this field is missing, if it is not a JSON array, or if it is an empty array, the validator MUST stop validation at this point and issue the failure code `cawg.ica.verified_identities.missing`.

The validator SHALL inspect each entry in the `verifiedIdentities` array. For each entry, it SHALL verify the following conditions and issue the failure code `cawg.ica.verified_identities.invalid` if the condition is not met:

* Each entry MUST be a JSON map data type.
* Each entry MUST contain a `type` property. This value MUST be a non-empty string that is known to and supported by the validator.
* Each entry MAY contain a `name` property. If present, it MUST be a non-empty string. (If the `type` for this entry is `cawg.document_verification`, the `name` property MUST be present.)
* Each entry MAY contain a `username` property. If present, it MUST be a non-empty string. (If the `type` for this entry is `cawg.social_media`, the `username` property MUST be present.)
* Each entry MAY contain an `address` property. If present, it MUST be a non-empty string. (If the `type` for this entry is `cawg.crypto_wallet`, the `address` property MUST be present.)
* Each entry MAY contain a `uri` property. If present, it MUST be a valid URI as defined by link:https://datatracker.ietf.org/doc/html/rfc3986[RFC 3986]. (If the `type` for this entry is `cawg.web_site`, the `uri` property MUST be present.)
* Each entry MUST contain a `verifiedAt` property. This value MUST be a valid date-time as defined by link:https://datatracker.ietf.org/doc/html/rfc3339[RFC 3339]. This date-time MUST precede the `issuedAt` or `validFrom` value for the credential itself.
* Each entry MUST contain a `provider` property. This value MUST be a JSON map array and it MUST contain a `name` property with a non-empty string.

NOTE: TO DO: Check with Leonard: Is there a way to annotate `self#jumbf` URIs with field references _inside_ a specific assertion? In other words, can we point to the specific entry in `verifiedIdentities` that was accepted or not accepted?

NOTE: TO DO: More specific error reporting about specific conditions not found?

===== Status codes

NOTE: TO DO: Add table summarizing status code for ICA success and failure states.
