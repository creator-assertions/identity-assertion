...
A _<<_credential_holder,credential holder>>_ such as credential compromise or acting in bad faith that would lead to the credential being revoked.

When possible, the _<<_identity_assertion_consumer,identity assertion consumer>>_ should look for appropriate revocation lists and verify the current status of the credential. The exact mechanism will vary based on the credential type and will be specified in the appropriate subsection of xref:_credentials_signatures_and_validation_methods[xrefstyle=full]. If a credential is found to be revoked, this information should be prominently displayed in any user experience regarding the *<<_identity_assertion,identity assertion>>.*
