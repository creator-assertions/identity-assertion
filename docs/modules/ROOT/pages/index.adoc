...
A _verifiable credential_ used with an identity assertion MUST contain only a single `credentialSubject` and that `credentialSubject` MUST have an `id` value, which MUST be a valid _https://www.w3.org/TR/did-core/[W3C decentralized identifier]._
