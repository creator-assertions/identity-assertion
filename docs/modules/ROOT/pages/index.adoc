...
NOTE: TO DO: We’re no longer having the VC subject sign even a partial claim, but a list of assertions. Need to think through exactly what we’re presenting to the VC holder and how we describe it.

NOTE: TO DO: Discuss whether the VC for the claim is necessary. I’ve heard some feedback that having a VC for the claim would make it easier for a wallet application to have suitable information to present when fulfulling the presentation request. A counter-concern is that this is effectively redundant and would require the claim-generator’s identity to be expressed both in X.509 and VC. Need to balance these concerns.
...
==== Verifiable credential for list of assertions

NOTE: TO DO: Describe how to create VC for the list of assertions. Depends on outcome of TO DO item earlier regarding whether this VC is necessary and what we call the list of assertions.
