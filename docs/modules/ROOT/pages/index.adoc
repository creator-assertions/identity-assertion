...
* `expected_countersigners` _(optional),_ if present, describes the content of other *<<_identity_assertion,identity assertion>>* in the same _<<C2PA Manifest>>._ The method for calculating this field is described in xref:_calculating_the_expected_countersigners_value[xrefstyle=full].
...
signer-payload-map = {
  ...
  ? "expected_countersigners": [1* $expected-countersigner-map], ; array of expected identity assertion values
}
...
expected-countersigner-map = {
  "partial_signer_payload": $signer-payload-map ; signer_payload value from other identity assertion minus any `expected_countersigner` value
...
.. Calculate the `signer_payload` value for that *<<_identity_assertion,identity assertion>>.* Remove any `expected_countersigners` value from that `signer_payload` value and place that in the `partial_signer_payload` value of _this_ *<<_identity_assertion,identity assertion’s>>* `expected_countersigners` map.
...
. Once the above materials have been gathered, the `expected_countersigners` value SHOULD contain a CBOR array of CBOR maps, each containing the following fields for each *<<_identity_assertion,identity assertion>>*:
.. `partial_signer_payload` _(required)_ containing the expected `signer_payload`, omitting any `expected_countersigners` value.
...
==== Validating the `expected_countersigners` value

If the *<<_identity_assertion,identity assertion>>* contains an `expected_countersigners` value in `signer_payload`, it MUST be validated as follows:

. For every _other_ *<<_identity_assertion,identity assertion>>* that is present in the <<C2PA Manifest>>:
.. Find the `signer_payload` value in that *<<_identity_assertion,identity assertion>>*. Remove any `expected_countersigners` value that is found in that `signer_payload` value. This will be referred to in the rest of this process as the _partial signer payload._
.. Search the `expected_countersigners` list for an entry where `partial_signer_payload` matches the just-computed _partial signer payload._ If no such entry can be found, stop validating and return the `cawg.identity.unexpected_countersigner` error code.
