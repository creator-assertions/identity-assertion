...
*Version 1.0 Draft 08 April 2024* · xref:_version_history[]
...
** `role` _(optional)_ contains a description of the credential subject’s relationship to the _<<C2PA asset>>._ Possible role values are described in xref:_credential_subject_roles[xrefstyle=full].
...
signer-payload-map = {
  ...
  ? "role": tstr .size (1..max-tstr-length), ; a string defining the credential subject's relationship to this C2PA asset
}
----
...
Possible values for the `signer_payload.role` field are provided in xref:_credential_subject_roles[xrefstyle=full].
