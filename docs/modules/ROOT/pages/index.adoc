...
Version 1.x (adding W3C VC credentials with credential schema) *Draft 13 June 2024* · xref:_version_history[]
...
[#vc-credentialsubject-person]
Person::
The `person` property MUST be an array that MAY be present. If present, every item of `person` MUST contain information about the _<<_named_actor,named actor>>_ whose identity has been verified by a certain _<<Identity verifier,identity verifiers>>_. The method of verification is beyond the scope of this specification. Each item of `person` property MUST contain at least two child properties: `name` and `verifiedBy`.
+
.Example:
[source,json]
----
...
---- 
[#vc-credentialsubject-person-name]
Person name::
The `person[?].name` property MUST be present and MUST not be empty. The `person[?].name` property defines the _<<_named_actor,named actor’s>>_ name as it was verified by the _<<Identity verifier,identity verifiers>>_.

[#vc-credentialsubject-person-verifiedBy]
Person verification details::
The `person[?].verifiedBy` property MUST be an object and MUST be present. The `person[?].verifiedBy` property MUST contain details about the _<<_identity_verifier,identity verifier>>_ and the identity verification process. This specification mentions at least three properties that MAY be used to represent the person verification details: `id`, `name`, and `proof`.

[#vc-credentialsubject-person-verifiedBy-id]
Identity verifier ID:::
The `person[?].verifiedBy.id` MUST be present and MUST be a valid URI that, when dereferenced, MUST result in a proof of authenticity of the _<<_identity_verifier,identity verifier>>_. This proof of authenticity of the _<<_identity_verifier,identity verifier>>_ must not be confused with the proof of verification of the _<<_named_actor,named actor>>_

[#vc-credentialsubject-person-verifiedBy-name]
Identity verifier name:::
The `person[?].verifiedBy.name` MUST be present and MUST not be empty. The `person[?].verifiedBy.name` property is the name of the _<<_identity_verifier,identity verifier>>_.

[#vc-credentialsubject-person-verifiedBy-proof]
Identity verification proof:::
The `person.verifiedBy[?].proof` is an optional field. If present, it MUST be a valid URI that, when dereferenced, MUST result in the proof of authenticity of the  _<<_named_actor,named actor>>_, as per the verification process of the _<<_identity_verifier,identity verifier>>_. The content as well as the process are outside the scope of this specification.
...
IMPORTANT: At least one of `person` property and `identities` property MUST be present, as they represent _<<_named_actor,named actor>>_ identity to be bound to with the _<<C2PA asset>>_ being described. Having both missing makes the credential invalid as there is no identity to be bound to the _<<C2PA asset>>._
...
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/122[issue #12]): Revise above language to point out that name collisions can occur to organizations as well as individuals.
...
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/116[issue #116]): Refine discussion of above items and add specific guidance.
