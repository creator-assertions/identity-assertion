...
The `verifiedIdentities` property MUST be present and MUST be an array. Every item in the array MUST contain information about the _<<_named_actor,named actor>>_ as verified by the _<<_identity_assertion_generator,identity assertion generator>>_ or a service contacted by the _<<_identity_assertion_generator,identity assertion generator>>._
...
+
If the `type` of this verified identity is `cawg.crypto_wallet`, the `verifiedIdentities[?].username` property MUST be present and MUST be the unique alphanumeric string that can be used to identity the user’s crypto wallet.
