...
NOTE: TO DO: Add list of supported encryption types and the corresponding type codes. (Asking Drummond for list of encryption type names.)

If the `type` code is unsupported, the validator MUST issue the failure code `cawg.ica.unsupported_alg` but MAY continue validation.
