...
signer-payload-map = {
  ...
  ? "expected_claim": bstr, ; byte string containing hash of partial claim
  ? "expected_claim_generator: bstr, ; byte string containing hash of expected claim signer credential
}
----
...
. For the specific *<<_identity_assertion,identity assertion>>* being computed, replace the `hash` value with 32 zero-value (`0x00`) bytes.
...
The resulting hash should be used as the `expected_claim` value.
...
. Replace the `hash` value for the specific *<<_identity_assertion,identity assertion>>* being validated with 32 zero-value (`0x00`) bytes.
