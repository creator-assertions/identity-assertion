...
*Preliminary draft 20 November 2023*
...
An identity assertion MUST contain valid CBOR that satisfies the `identity` rule in xref:_schema_and_example[xrefstyle=full]. The `xyz.identity.cbor.invalid` error code SHALL be used to report assertions that do follow this rule.

The `pad1` and `pad2` fields of an identity assertion MUST contain only zero bytes. The `xyz.identity.pad.invalid` error code SHALL be used to report assertions that contain other values in these fields.
...
In some use cases, the _<<actor,actors>>_ in the system may wish to provide their own https://www.w3.org/TR/vc-data-model/[W3C verifiable credential], as they exist at that moment in time, to the _claim generator_ to have them associated with one or more _<<_c2pa-assertion,C2PA assertions>>._ These actors may be individuals, groups, or organizations.
...
The actor’s _verifiable credential_ is combined with a _verifiable credential_ for the _<<C2PA claim>>_ into a link:++https://www.w3.org/TR/vc-data-model/#presentations-0++[W3C verifiable presentation].

NOTE: Discuss whether the VC for the claim is necessary. I’ve heard some feedback that having a VC for the claim would make it easier for a wallet application to have suitable information to present when fulfulling the presentation request. A counter-concern is that this is effectively redundtant and would require the claim-generator’s identity to be expressed both in X.509 and VC. Need to balance these concerns.
...
NOTE: JSON-LD serialization is mandated as it is the most commonly used of the three syntaxes presented in section 6 of the W3C verifiable credentials specification. It is also the one that aligns best with its extensibility model, which could be useful to some implementers.
...
A W3C verifiable credential used with an identity assertion MUST contain only a single `credentialSubject` and that `credentialSubject` shall have an `id` value, which MUST be a valid https://www.w3.org/TR/did-core/[W3C Decentralized Identifier].
...
All W3C distributed identifiers that are stored in an identity assertion SHALL be generated using one of the DID methods as described in this section. This section defines both:

* A list of DID methods that are allowed for generating identifiers for new content as well as required for validating identifiers of existing content (the allowed list), and
* A list of DID methods that are required to be supported for validating identifiers of existing content but are not allowed for generating identifiers for new content (the deprecated list).

A DID method MUST appear in no more than one list. If a DID method does not appear in either list, it is forbidden and must not be used or supported. Algorithms can be removed from the lists in order to implement forbidding an algorithm. For this reason, implementations MUST NOT support additional algorithms on an optional basis.
...
In some use cases, the _<<actor,actors>>_ in the system may wish to provide an https://tools.ietf.org/html/rfc5280[X.509 certificate] to have an organizational or individual identity described by the certificate associated with the _<<_c2pa-claim,C2PA claim>>._

The `sig_type` value for such an assertion MUST be `org.ietf.cose`. The `signature` value MUST be a https://datatracker.ietf.org/doc/html/rfc8152[COSE signature] as described below.
...
* Any reference to the _claim generator_ MUST be replaced with the _actor_ whose _X.509 certificate_ is being used for this assertion.
