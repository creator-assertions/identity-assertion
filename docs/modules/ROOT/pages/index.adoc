...
*Pre-draft 02 February 2024* · xref:_version_history[]
...
For each entry in the `referenced_assertions` field, the validator MUST verify that the same entry exists in the `assertions` entry of the _<<C2PA claim>>._ The `cawg.identity.assertion.mismatch` error code SHALL be used to report violations of this rule.
