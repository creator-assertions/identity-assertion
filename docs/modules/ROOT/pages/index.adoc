...
*Preliminary draft 01 December 2023* +
xref:_version_history[]

NOTE: This draft includes a proposed additional requirement for _DID verification methods._ See xref:_did_verification_methods[xrefstyle=full].
...
A temporary _<<C2PA assertion>>_ that is created during _<<C2PA claim>>_ generation which reserves space for the eventual content of identity assertion. A placeholder assertion MUST be used when the final file layout of the _<<C2PA asset>>_ is required for the content binding assertion.
...
* https://www.w3.org/TR/did-core/[W3C distributed identifiers (DIDs), version 1.0]
...
Each identity assertion instance allows exactly one _<<_credential_subject,credential subject>>_ to sign a list of _<<_c2pa_assertion,C2PA assertions>>_ which will subsequently be included in a _<<C2PA claim>>._

The identity assertion contains the following data fields:

* `assertions` contains a list of assertions
...
The content of `assertions` is a list of assertions which is presented to the _<<_credential_subject,credential subject>>_ for signature. This list of assertions MUST include a content binding assertion as described in link:++https://c2pa.org/specifications/specifications/1.4/specs/C2PA_Specification.html#_hard_bindings++[Section 10.2, “Hard Bindings,”] of the C2PA technical specification.

For each assertion listed in the `assertions` field of the identity assertion, an assertion with the same `url`, `alg`, and `hash` values MUST also be listed in the `assertions` field of the _<<C2PA claim>>_ in which the identity assertion appears.

The `assertions` entry of the identity assertion MUST NOT reference the same identity assertion itself.

NOTE: The list of assertions in the `assertions` field of the identity assertion is necessarily a subset of the assertions listed in the `assertions` field of the _<<C2PA claim>>_ because the identity assertion may not refer to itself. The list of assertions may be further subsetted. For example, different _<<_credential_subject,credential subjects>>_ could attest to different assertions in a single _<<C2PA Manifest>>._

The `assertions` entry of the identity assertion MUST NOT reference any assertion more than once.

The `assertions` entry of an identity assertion MAY reference any other identity assertion. A claim generator MUST NOT allow any two identity assertions to refer to each other.
...
=== Presenting the list of assertions for signature

The list of assertions that will be included in the identity assertion MUST be presented to be signed by the _<<_credential_subject,credential subject>>_ corresponding to each identity assertion. The exact formatting of the list of assertions as presented to the signature method will depend on the nature of credential presented. These are described in xref:_credentials_signatures_and_validation_methods[xrefstyle=full].

=== Creating the assertion

IMPORTANT: If a data hash assertion is being used, the claim generator MUST also follow the process described in xref:_interaction_with_data_hash_assertion[xrefstyle=full].
...
The `assertions` field MUST contain the same list of assertions that was presented to the _<<_credential_subject,credential subject>>_ for signature.

The acceptable values for the `sig_type` and `signature` fields depend on the nature of credential used and are described in xref:_credentials_signatures_and_validation_methods[xrefstyle=full]. If a _<<placeholder_assertion,placeholder assertion>>_ was used and the signature exceeds the space available in the _<<placeholder_assertion,placeholder assertion,>>_ the claim generation process MUST be repeated with a larger padding size chosen in the xref:_creating_placeholder_assertions[xrefstyle=full] step.

If a _<<placeholder_assertion,placeholder assertion>>_ was used, the values of the `pad1` and `pad2` fields MUST now be recomputed (adjusted in size) such that the resulting identity assertion exactly matches the size in bytes of the _<<placeholder_assertion,placeholder assertion.>>_
...
Similarly, the size of the identity assertion cannot be known until its signature is obtained. Changing the size of the identity assertion after file layout is completed would invalidate the content binding hash, which is required to be present in the identity assertion.
...
When using a data hash assertion, a claim generator MUST follow the process described in link:++https://c2pa.org/specifications/specifications/1.4/specs/C2PA_Specification.html#_creating_a_claim++[Section 11.3, “Creating a claim,”] and link:++https://c2pa.org/specifications/specifications/1.4/specs/C2PA_Specification.html#_multiple_step_processing++[Section 11.4, “Multiple step processing,”] of the C2PA technical specification with additional steps as described below:
...
sequenceDiagram
    ...
    G->>H: Request signature over list of assertions
    H->>G: Provide signature over list of assertions
    end
...
For each entry in the `assertions` field, the validator MUST verify that the same entry exists in the `assertions` entry of the _<<C2PA claim>>._ The `cca.identity.assertion.mismatch` error code SHALL be used to report violations of this rule.

The validator MUST ensure that the `assertions` field contains at least one content binding assertion as described in link:++https://c2pa.org/specifications/specifications/1.4/specs/C2PA_Specification.html#_hard_bindings++[Section 10.2, “Hard Bindings”] of the C2PA technical specification. The `cca.identity.content_binding_missing` error code SHALL be used to report a missing content binding.
...
|`cca.identity.content_binding_missing` | The identity assertion does not reference a content binding assertion.
...
This _DID document_ MUST contain an `assertionMethod` field as defined by link:++https://www.w3.org/TR/did-core/#assertion__[Section 5.3.2, “Assertion,”] of the W3C decentralized identifiers specification. Additional requirements for this verification method are stated in _xref:_did_verification_methods[xrefstyle=full]._

NOTE: TO DO: Clean up this next paragraph. Running out of time for today. 

The _claim generator_ MUST prepare a list of _<<_c2pa_assertion,C2PA assertions>>_ into a (WHAT? Verifiable Presentation Request) to be presented to the _<<_actor,actor.>>_ The _<<_actor,actor>>_ MUST respond with a Verifiable Presentation (TO DO: Add link) containing (TO DO: in what fields?) the VC and list of assertions.

NOTE: TO DO: Does the list of assertions itself need a VC? If so, who issues that VC? (That feels redundant with the separate chain of authority that goes into the claim itself via X.509.)

The relationships among _<<_actor,actor,>>_ _verifiable credential,_ _DID document,_ _assertions,_ and the _verifiable presentation_ are illustrated in the following diagram, which is non-normative.
...
Once the verifiable presentation has been obtained, an identity assertion can be formed. The `sig_type` value for such an assertion MUST be `org.w3.verifiable_presentation` and the `signature` value MUST be the _verifiable presentation_ serialized as described in link:++https://www.w3.org/TR/vc-data-model/#json-ld++[Section 6.2, “JSON-LD,”] of the W3C verifiable credentials data model spec.
...
NOTE: TO DO: Reconsider the following statement from C2PA 1.3 spec. Since we’re shifting to verifiable presentations, I think we want to make a stronger statement to indicate that the actor _did_ authorize the assertions.
...
NOTE: TO DO: This phrasing is awkward. Is there a better way to specify that this standard requires implementations to choose from a limited set of verification methods?

All _W3C verification methods_ that are stored in a _DID document_ referenced from an identity assertion SHALL be one of the _verification methods_ as described in this section. This section defines both:

* A list of _verification methods_ that are allowed when generating new content as well as required for validating public key material for existing content (the allowed list), and
* A list of _verification methods_ that are required to be supported for validating public key material for existing content but are not allowed for generating new content (the deprecated list).

A _verification method_ MUST appear in no more than one list. If a _verification method_ does not appear in either list, it is forbidden and MUST NOT be used or supported. Algorithms can be removed from the lists in order to implement forbidding an algorithm. For this reason, implementations MUST NOT support additional algorithms on an optional basis.

NOTE: TO DO: Is the strict prohibition necessary in this case?

Implementers SHOULD consult this section in the current version of the specification when releasing software updates and ensure their supported algorithms conform to it.

NOTE: Refer to link:++https://www.w3.org/TR/did-spec-registries/#verification-method-types++[Section 6.1, “Verification method types,”] of the W3C DID specification registries document for further information on verification method implementation details.
...
NOTE: TO DO: Solicit feedback on other verification methods that should be allow-listed.
...
NOTE: TO DO: Describe how to create the VP from the above two credentials.
...
In some use cases, an _<<_actor,actor>>_ in the system may wish to provide an _https://tools.ietf.org/html/rfc5280[X.509 certificate]_ to have an organizational or individual identity described by the certificate associated with the list of _<<_c2pa_assertion,C2PA assertions>>._
...
* Any reference to the _claim_ MUST be replaced with the CBOR serialization of the `assertions` field from the identity assertion.
