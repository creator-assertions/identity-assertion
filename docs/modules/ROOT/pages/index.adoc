...
==== Calculating the `expected_partial_claim` value

If the *<<_identity_assertion,identity assertion>> generator* chooses to include the `expected_partial_claim` value in `signer_payload`, it MUST be computed as follows:
...
The `expected_partial_claim` value SHOULD contain a CBOR map with the following fields:
...
==== Validating the `expected_partial_claim` value

If the *<<_identity_assertion,identity assertion>>* contains an `expected_partial_claim` value in `signer_payload`, it MUST be validated as follows:
