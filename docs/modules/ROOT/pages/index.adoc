...
* `role` _(optional)_ contains one or more descriptions of the credential subject’s relationships to the _<<C2PA asset>>._ Possible role values are described in xref:_credential_subject_roles[xrefstyle=full].
...
==== Credential subject roles

The _<<_credential_holder,credential holder>>_ MAY choose to describe one or more relationships of the _<<_credential_subject,credential subject>>_ to the _<<C2PA asset>>_ by adding a `role` field to `signer_payload`.
...
|=======================
| Value        |  Meaning
| `cawg.creator` | The _<<_credential_subject,credential subject>>_ is a primary creator/author of this _<<C2PA asset>>._
| `cawg.contributor` | The _<<_credential_subject,credential subject>>_ is a secondary creator/author of this _<<C2PA asset>>._
| `cawg.editor` | The _<<_credential_subject,credential subject>>_ is an editor of this _<<C2PA asset>>._
| `cawg.producer` | The _<<_credential_subject,credential subject>>_ is a producer of this _<<C2PA asset>>._
| `cawg.publisher` | The _<<_credential_subject,credential subject>>_ is a publisher of this _<<C2PA asset>>._
| `cawg.reviewer` | The _<<_credential_subject,credential subject>>_ is a reviewer of this _<<C2PA asset>>._
| `cawg.sponsor` | The _<<_credential_subject,credential subject>>_ has supported or sponsored the creation of this _<<C2PA asset>>._
...
signer-payload-map = {
  ...
  ? "role": [1* tstr .size (1..max-tstr-length)], ; one or more strings defining the credential subject's relationship(s) to this C2PA asset
}
----
...
Possible values for the text string entries in the `signer_payload.role` field are provided in xref:_credential_subject_roles[xrefstyle=full].
