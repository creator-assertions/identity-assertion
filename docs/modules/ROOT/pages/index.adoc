...
.. Calculate the `signer_payload` map for that *<<_identity_assertion,identity assertion>>.* Remove any `expected_countersigners` field from that `signer_payload` map and place that in the `partial_signer_payload` field of _this_ *<<_identity_assertion,identity assertion’s>>* `expected_countersigners` map.
