...
An organization or system that is responsible with confirming the authenticity and validity of an individual's or entity's identity. 
...
A *creator indentity assertion* is a _<<_w3c_verifiable_credential,W3C verifiable credential>>_ that binds toghether the identity of an _<<_actor,actor>>_  with their _<<C2PA asset>>_ contribution. Below are depicted the requirements that a *creator indentity assertion* must satisfy.
...
* String format
+
When specified as a string, the `issuer` MUST specify a valid URI that, when dereferenced, results in a document that can be used to verify the information from the credential, as specified by the https://www.w3.org/TR/vc-data-model-2.0/#issuer[4.7 Issuer] section of the https://www.w3.org/TR/vc-data-model-2.0/[Verifiable Credentials Data Model]. 
+
.Example:
[source,json]
...
Validity::
The `validFrom` field MUST exist and it MUST express the date and time when the credential becomes valid, as specified by the https://www.w3.org/TR/vc-data-model-2.0/#validity-period[4.8 Validity Period] section of the https://www.w3.org/TR/vc-data-model-2.0/[Verifiable Credentials Data Model].

*Creator identity properties*

The `credentialSubject` property MUST contain both the identity of _<<_actor,actor>>_ and the _<<C2PA asset>>_ contribution. These can be represented by the following properties:

[#vc-credentialsubject-id]
Id::
The `id` MUST be present and MUST represent an URI. It is RECOMMENDED that the URI be one which, if dereferenced, results in a globally unambiguos status of existence of the  _<<_actor,actor>>_. For example, dereferencing a `did:web` might return a `204 No Content` HTTP Status Code in case the user exists and no further information is available or `404 Not Found` HTTP Status Code in case the user does not exist.
`
[#vc-credentialsubject-person]
Person::
The `person` property MUST be an object that MAY be present. If present `person` MUST contain information about the _<<_actor,actor>>_ whose identity has been verified. The method of verification is beyond the scope of this specification. The `person` property MUST contain at least two child properties: `name` and `verifiedBy`.
+
.Example:
[source,json]
----
...
Person name::
The `person.name` property MUST be present and MUST not be empty. The `person.name` property defines the _<<_actor,actor>>_ name as it was verified by one or multiple _<<Identity verifier,Identity verifiers>>_.

[#vc-credentialsubject-person-verifiedBy]
Person verification details::
The `person.verifiedBy` property MUST be a non empty array and MUST be present. Each item of the array MUST contain details about the _<<Identity verifier>>_  and the identity verification process. This specification mentions at least three properties that may be used to represent the person verification details: `id`, `name` and `proof`.

[#vc-credentialsubject-person-verifiedBy-id]
Identity Verifier id:::
The `person.verifiedBy[?].id` MUST be present and MUST be a valid URI that, when dereferenced, MUST result in a proof if authenticity of the _<<Identity verifier>>_.

[#vc-credentialsubject-person-verifiedBy-name]
Identity Verifier name:::
The `person.verifiedBy[?].name` MUST be present and MUST not be empty. The `person.verifiedBy[?].name` property is the name of the _<<Identity verifier>>_.

[#vc-credentialsubject-person-verifiedBy-proof]
Identity Verification proof:::
The `person.verifiedBy[?].proof` MAY be present and MUST be a valid URI. The URI, when dereferenced, MUST result in the proof of authenticity of the  _<<_actor,actor>>_, as per the verification process of the _<<Identity verifier>>_. The content as well as the process are outside the scope of this specification.

[#vc-credentialsubject-identities]
Identities::
The `identities` property MUST be an array that MAY be present. It contains information about the accounts that the _<<_actor,actor>>_ allowed to be bound to with their _<<C2PA asset>>_ contribution.
+
.Example:
[source,json]
----
...
This identity represention presents the details of a social account of the _<<_actor,actor>>_. How the social accounts are verified and associated with the _<<_actor,actor>>_ are outside the scope of this specifiation. An idenity by social provider needs to contain at least the following five child properties: `type`, `username`, `url`, `provider`, `createdAt`.
...
Identity username:::
The `identities[?].username` MUST be present and MUST not be empty. It represents the social account's username.

[#vc-credentialsubject-identities-url]
Identity url:::
The `identities[?].url` MUST be present and MUST be a valid URI. It represents an URL that can prove the existence of an account for the username.
...
The identities[?].createdAt` MUST be present and MUST be a valid date-time as specified by https://datatracker.ietf.org/doc/html/rfc3339[RFC 3339]. It represents the date and time when this social account was associated with the _<<_actor,actor>>_. 

Identity by crypto wallet::
This identity representation presents the details of a crypto wallet account of the _<<_actor,actor>>_. How the crypto wallet accounts are verified and associated with the _<<_actor,actor>>_ are outside the scope of this specifiation. An identity by crypto wallet needs to contain at least the following child properties: `type`, `username`, `address`, `provider`, `createdAt`.

Identity by crypto wallet type:::
In this case the `identities[?].type` property MUST be present and MUST be `cryptoWallet`

Identity username:::
The `identities[?].username` MUST be present and MUST not be empty. It represents the crypto wallet account's username.

Identity address:::
The `identities[?].address` MUST be present and MUST be valid crypto wallet address. It represents an URL that can prove the existence of an account for the username.

Identity provider:::
The `identities[?].provider` MUST be present. It represents a unique alphanumeric string that can be used to identity the user. 

Identity creation date:::
The identities[?].createdAt` MUST be present and MUST be a valid date-time as specified by https://datatracker.ietf.org/doc/html/rfc3339[RFC 3339]. It represents the date and time when this crypto wallet account was associated with the _<<_actor,actor>>_. 

IMPORTANT: At least one of `person` property and `identities` property MUST be present, as they represent _<<_actor,actor>>_ identites to be bound to with their _<<C2PA asset>>_ contribution. Having both missing makes the credential invalid as there is no identity to be bound to the _<<C2PA asset>>_ contribution.
