...
Version 1.x (adding W3C VC credentials) *Draft 12 August 2024* · xref:_version_history[]
...
An _<<_actor,actor>>_ MAY wish to provide a service API for providing *<<_identity_assertion,identity assertion>>* signatures and credentials. This MAY be done via any protocol. This standard defines a RECOMMENDED API which can be made available via link:https://datatracker.ietf.org/doc/html/rfc9112[HTTP 1.1 (RFC 9112)] or any subsequent version of the HTTP protocol.

The intent of this API is that a software library can be configured by providing a single HTTPS endpoint address. All remaining configuration can be discovered via that endpoint.

This standard describes three endpoints:

* *Configuration and discovery:* Provides additional information required for a client process to perform the remaining operations.
* *User-specific options:* Provides additional information about an authenticated user which can be used to configure subsequent signing requests.
* *Generate asset-specific credential:* Given a description of a specific _<<C2PA asset>>_ and a specific authenticated user, generate an appropriate `signature` value binding the user to the asset.

The interaction between _<<_identity_assertion_generator,identity assertion generator>>_ and the service API is depicted in the following sequence diagram, which is non-normative:

[mermaid,width=100%]
....
sequenceDiagram
    participant IAG as Identity assertion generator
    participant API as Service API

    IAG->>API: Request configuration information
    API->>IAG: Respond with configuration information
    Note right of IAG: Caching of this information is encouraged

    IAG->>API: Request user-specific options
    API->>IAG: Respond with user-specific options
    Note right of IAG: This may inform user experience<br>such as which verified<br>identities are available

    IAG->>IAG: Generate signer_payload
    IAG->>API: Request signature for user and signer_payload
    API->>IAG: Generate signature

    IAG->>IAG: Embed signature in identity assertion
....

=== Configuration and discovery

This request is an HTTPS `GET` with no parameters.

The endpoint address is communicated out-of-band from the API service provider to the _<<_identity_assertion_generator,identity assertion generator>>._

This request MAY require authentication via a `Bearer` token.

This request provides basic information about the remainder of the service.

If the API service is stable (and, if applicable, the authentication token is valid), the service SHALL respond with HTTP status 200 (OK), a `Content-Type` header of `application/json`, and a JSON object conforming to the `service-description` rule in the following https://datatracker.ietf.org/doc/html/rfc8610[CDDL definition]:

[source,cddl]
----
service-description = {
  "endpoints": $endpoint-map, ; subsequent 
  "name": tstr .size (1..max-tstr-length), ; user-visible name for this service
}

endpoint-map = {
  "user_options": tstr .size (1..max-tstr-length), ; HTTPS endpoint address for user-specific options
  "sign": tstr .size (1..max-tstr-length), ; HTTPS endpoint address for generating asset-specific credential
}
----

.Example service description response
[sidebar]
****
This example, which is non-normative, depicts a possible response from the configuration and discovery API.

[source,json]
----
{
  "endpoints": {
    "user_options": "https://api.example.com/user_options",
    "sign": "https://api.example.com/sign",
  },
  "name": "Example Identity Assertion Signing Service"
}
----
****

IMPORTANT: Service and client implementers are encouraged to use HTTP caching options as defined by link:https://datatracker.ietf.org/doc/html/rfc9111[RFC 9111] to avoid repetitive traffic on this endpoint.

=== User-specific options

This request is an HTTPS `GET` with no parameters.

The endpoint address is obtained via the `endpoints.user_options` member of the xref:_configuration_and_discovery[xrefstyle=full] response.

This request MUST require authentication via a `Bearer` token.

This request provides configuration information that is specific to the authenticated user on the service.

If the API service is stable and the authentication token is valid, the service SHALL respond with HTTP status 200 (OK), a `Content-Type` header of `application/json`, and a JSON object conforming to the `user-options` rule in the following https://datatracker.ietf.org/doc/html/rfc8610[CDDL definition]:

[source,cddl]
----
user-options = {
  "sig_type": tstr .size (1..max-tstr-length), ; signature type
  "suggested_size: uint, ; suggested size for signature placeholder
  "verified_identities": $verified-identity-map, ; available verified identities for user to choose
  "identity_required": bool, ; true if user must choose at least one identity
}
----

.Example user-specific options response
[sidebar]
****
This example, which is non-normative, depicts a possible response from the user-specific options API.
...
=== Generate asset-specific credential

This request is an HTTPS `POST`.

The endpoint address is obtained via the `endpoints.sign` member of the xref:_configuration_and_discovery[xrefstyle=full] response.

This request MUST require authentication via a `Bearer` token.

The request endpoint MAY contain a query parameter named `identities`. If present, this item must BE a comma-separated list of verified identities. Each name specified in this query string MUST be a name provided by this service via the `verified_identities[?].name` member of the xref:_user_specific_options[xrefstyle=full] response.

NOTE: TO DO: Figure out how to map names to verified identities. The `verified_identities[?].name` isn't actually the key we need.

The request MUST contain a `Content-Type` header of `application/cbor` and the request body MUST be the `signer_payload` structure, encoded as described by link:++https://www.rfc-editor.org/rfc/rfc8949.html#name-core-deterministic-encoding++[Section 4.2.1, “Core deterministic encoding,”] of RFC 8949: Concise binary object representation.

If the API service is stable, the authentication token is valid, and the `signer_payload` is acceptable, the service SHALL respond with HTTP status 200 (OK), a `Content-Type` header of `application/octet-stream`. The response body MUST be the exact value to place in the `signature` field of the *<<_identity_assertion,identity assertion>>.* (The data type may vary depending on the `sig_type` value.)

=== Error responses

In the event the service is unable to respond successfully to any of the requests described in this section, the service SHOULD respond with an appropriate HTTP status code (4xx or 5xx). This specification does not specify the response body for error cases.
