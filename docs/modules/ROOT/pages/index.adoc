...
Version 1.x (adding W3C VC credentials) *Draft 25 July 2024* · xref:_version_history[]
...
==== Creator identity assertion description

A *creator identity assertion* is a _<<_w3c_verifiable_credential,W3C verifiable credential>>_ that binds the identity of the _<<_named_actor,named actor>>_ to the _<<C2PA asset>>_ in which the *<<_identity_assertion,identity assertion>>* appears. A *creator identity assertion* MUST meet all requirements for a verifiable credential as described in https://www.w3.org/TR/vc-data-model-2.0/[Verifiable credentials data model, version 2.0], and additional requirements as stated in the remainder of this section:

IMPORTANT: At least one of `namedActor` property and `identities` property MUST be present, as they represent the _<<_named_actor,named actor’s>>_ identity to be bound to the _<<C2PA asset>>_ being described. Having both missing makes the credential invalid as there is no identity to be bound to the _<<C2PA asset>>._
...
[#issue-145]
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/145[issue #145]): Transition the creator identity assertion context to the recently-acquired domain cawg.io once it is provisioned and ready to use.
...
The `issuer` property MUST be present and MAY have one of two representations:

* *String format:* When specified as a string, the `issuer` MUST specify a valid URI that, when dereferenced, results in a document that can be used to verify the information from the credential, as specified in link:++https://www.w3.org/TR/vc-data-model-2.0/#issuer++[Section 4.7, “Issuer,”] of the Verifiable credentials data model.
+
.Example:
[source,json]
"issuer": "did:example:2g55q912ec3476eba2l9812ecbfe"

* *Object format:* When specified as an object, the `issuer` MUST include an `id` property. The `id` property MUST be a valid URI that, when dereferenced, results in a document that can be used to verify the information from the credential, as specified in link:++https://www.w3.org/TR/vc-data-model-2.0/#issuer++[Section 4.7, “Issuer,”] of the Verifiable credentials data model.
...
[#pr-146]
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/pull/146[PR #146]): Consider removing this section. I don’t think it adds any new requirements over the existing W3C VC requirements.
...
[#pr-147]
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/pull/147[PR #147]): Consider removing this section. This section doesn't add much information over the existing W3C VC requirements except the requirement to have an `id` field and I'm not sure that's actually necessary.

[#vc-credentialsubject-namedactor]
===== Named actor

The `namedActor` property is an OPTIONAL property within the `credentialSchema` property. If present, it MUST be an array and every item MUST contain information about the _<<_named_actor,named actor>>_ whose identity has been verified by a certain _<<Identity verifier,identity verifiers>>_. The method of verification is beyond the scope of this specification. Each item of `namedActor` property MUST contain at least two child properties: `name` and `verifiedBy`.
...
[#issue-148]
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/148[issue #148]): Consider merging this down into `credentialSubject` and making verifications its own property.
...
The `affiliations` property is an OPTIONAL field within the `credentialSchema` property. If present, it MUST be an array in which each item MUST contain information about an affiliation of the _<<_named_actor,named actor>>._ The process of proving the validity of these affiliations is outside the scope of this specification. This specification mentions at least three properties that MAY be used to represent the _<<_named_actor,named actor’s>>_ affiliation details: `id`, `name`, and `proof`.
...
[#issue-149]
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/149[issue #149]): Define what we mean by "proof of authenticity" here.
...
The `identities` property is an optional field within the `credentialSchema` property. If present, it MUST be an array containing information about the accounts that the _<<_named_actor,named actor>>_ requested to be bound with the _<<C2PA asset>>_ being described.
...
----
This specification mentions two identity types that MAY be used, but this specification is not limited to these two:

* `socialProvider`
* `cryptoWallet`

[#issue-150]
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/150[issue #150]): Consider changing the identity type names to `snake_case` for consistency with the rest of the spec. Consider using `cawg.` prefix to allow non-CAWG identity types to be used here.
...
The `credentialSubject` field MUST contain a `boundTo` entry, which MUST be the JSON serialization of the `signer_payload` data structure presented for signature. All bytestring entries in the `signer_payload` data structure (i.e. hash entries in hashlink URL data structures) MUST be converted to the corresponding Base-64 plain-text strings.

[#issue-151]
[NOTE]
====
TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/151[Issue #151]): Review description of `boundTo` property.

* Specify Base-64 encoding options.
* Consider name of this property. I'm not sure `boundTo` conveys the meaning fully.
* Decide whether to preserve `snake_case` names or translate to `camelCase` for `signer_payload` content. Current example uses `camelCase`.
====
...
An example of the *<<_creator_identity_assertion,creator identity assertion>>* verifiable credential is given below:

.Example creator identity assertion verifiable credential
[#example-ia-vc]
[source,json]
----
{
...
The inclusion of the `creator-identity-assertion.json` schema in the verifiable credential `credentialSchema` is OPTIONAL. However, it is RECOMMENDED to include it as it enforces this specification’s constraints.
...
[#issue-155]
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/155[issue #155]): Write section on proofs.
