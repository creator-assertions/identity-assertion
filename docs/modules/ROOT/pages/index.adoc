...
IMPORTANT: TO DO: *Major discussion item:* I think there are reasonable cases for the content of the *<<_identity_assertion,identity assertion>>* to be a new Verifiable Credential and also for it to be a Verifiable Presentation. The VC path will resolve some of the redundant-chain-of-trust issues identified below, but may complicate the signing experience for wallet-based applications. Currently assuming Verifiable Presentation, but this is subject to change.
...
The _<<C2PA claim generator>>_ prepares the list of _<<_referenced_assertions,referenced assertions>>_ to be presented to the _<<_credential_subject,credential subject>>,_ which MUST in turn respond with a *verifiable presentation* as defined in link:++https://www.w3.org/TR/vc-data-model/#presentations-0++[Section 4.10, “Presentations,”] of the W3C verifiable credentials data model. Additional requirements for this presentation are stated in xref:_verifiable_presentation_requirements[xrefstyle=full].
...
The relationships among _<<_actor,actor,>>_ _<<W3C verifiable credential>>,_ _<<W3C decentralized identifier document>>,_ _<<_referenced_assertions,referenced assertions>>,_ and the _verifiable presentation_ are illustrated in the following diagram, which is non-normative:
...
Once the verifiable presentation has been obtained, the *<<_identity_assertion,identity assertion>>* can be formed. The `sig_type` value for such an assertion MUST be `org.w3.verifiable_presentation` and the `signature` value MUST be the _verifiable presentation_ serialized as described in link:++https://www.w3.org/TR/vc-data-model/#json-ld++[Section 6.2, “JSON-LD,”] of the W3C verifiable credentials data model spec.
...
NOTE: TO DO: Specify structure, including how list of _<<_referenced_assertions,referenced assertions>>_ is expressed, for VP.

==== Validating an identity assertion with a W3C verifiable presentation

An *<<_identity_assertion,identity assertion>>* with `sig_type` of `org.w3.verifiable_presentation` is only valid if the following conditions have been successfully verified:
...
* The list of _<<_referenced_assertions,referenced assertions>>_ in the verifiable presentation must match the assertions referenced in the `referenced_assertions` field of the *<<_identity_assertion,identity assertion>>* structure.
* The _<<W3C decentralized identifier document>>_ referenced in the verifiable presentation is successfully resolved and contains public key cryptographic material as required by xref:_did_document_requirements.
* The `proof` in the verifiable presentation indicated a valid signature for the public key cryptographic material in the _<<W3C decentralized identifier document>>_ and the list of _<<_referenced_assertions,referenced assertions>>._
