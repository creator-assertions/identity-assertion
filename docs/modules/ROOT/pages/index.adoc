...
* `tbs` (“to be signed”) contains a set of data to be signed by this _<<_credential_subject,credential subject>>_. This structure contains:
** `referenced_assertions` contains a list of other assertions selected by the credential subject. This is referred to as `tbs.referenced_assertions` throughout this specification.
...
The content of `tbs` is a data structure which is presented to the _<<_credential_subject,credential subject>>_ for signature. The list of _<<_referenced_assertions,referenced assertions>>_ contained within `tbs` MUST include a _<<hard_binding,hard binding>>_ assertion as described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_hard_bindings++[Section 9.2, “Hard bindings,”] of the C2PA technical specification.

IMPORTANT: A single signature from the _<<_credential_subject,credential subject>>_ covers the entire `tbs` data structure, including the _list_ of assertions. An implementation MUST NOT present each assertion separately for signature.
...
Prior to requesting the credential holder with the `tbs` data structure for signature, the _<<_referenced_assertions,referenced assertions>>_ MUST themselves be created. This process is described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_creating_assertions#_creating_assertions++[Section 10.3.1, “Creating assertions,”] of the C2PA technical specification.
...
The `tbs` data structure MUST be presented to be signed by the _<<_credential_subject,credential subject>>_ corresponding to each *<<_identity_assertion,identity assertion>>.* This process is described in more detail in xref:_credentials_signatures_and_validation_methods[xrefstyle=full].
...
The `tbs` data structure MUST be unchanged from the data structure that was presented to the _<<_credential_subject,credential subject>>_ for signature.
