...
Version 1.x (adding W3C VC credentials) *Pre-Draft 20 March 2024* · xref:_version_history[]
...
IMPORTANT: A single signature from the _<<_credential_holder,credential holder>>_ covers the _list_ of assertions. An implementation MUST NOT present each assertion separately for signature.
...
Prior to requesting the _<<_credential_holder,credential holder>>_ with the list of _<<_referenced_assertions,referenced assertions>>_ for signature, the assertions to be signed MUST themselves be created. This process is described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_creating_assertions#_creating_assertions++[Section 10.3.1, “Creating assertions,”] of the C2PA technical specification.
...
The list of _<<_referenced_assertions,referenced assertions>>_ MUST be presented to be signed by the _<<_credential_holder,credential holder>>_ corresponding to each *<<_identity_assertion,identity assertion>>.* This process is described in more detail in xref:_credentials_signatures_and_validation_methods[xrefstyle=full].
...
====
This section assumes that the following pending pull requests in 1.0-draft will be merged:

* link:https://github.com/creator-assertions/identity-assertion/pull/75[PR #75: Add top-level `tbs` map allowing more content to be signed by credential holder]
* link:https://github.com/creator-assertions/identity-assertion/pull/82[PR #82: Use assertion label to describe credential type]
====

In some use cases, a _<<_credential_holder,credential holder>>_ in the system MAY wish to provide a _<<W3C verifiable credential>>,_ as it exists at that moment in time, to the _<<C2PA claim generator>>_ to have it associated with one or more _<<_c2pa-assertion,C2PA assertions>>._ The corresponding _<<_credential_subject,credential subject>>_ MAY be an individual, group, or organization.

The assertion label for such an assertion MUST be `cawg.identity.vc`.

The _<<_credential_holder,credential holder’s>>_ _<<W3C verifiable credential>>_ is not itself directly embedded in the *<<_identity_assertion,identity assertion>>,* but is instead used to issue a new _<<W3C verifiable credential>>_ describing the _<<_credential_subject,credential subject’s>>_ role in the specific _<<C2PA asset>>_ being described.

The relationships among _<<_credential_holder,credential holder’s>>_ _<<W3C verifiable credential>>,_ _<<W3C decentralized identifier document>>,_ *identity assertion’s* _<<W3C verifiable credential>>,_ the `tbs` data structure, and the _<<_c2pa_asset,C2PA asset>>_ are illustrated in the following diagram, which is non-normative:

[mermaid,width=100%]
....
erDiagram
  CH["Credential holder"] {}
  CHVC["Credential holder’s verifiable credential"] {}
  TBS["`tbs` data structure"] {}
  IVC["Identity assertion’s verifiable credential"] {}
  ASSET["C2PA asset"] {}

  CH ||--|| CHVC : "controls via private key"
  IVC||--|| TBS: "references"
  IVC ||--|| CHVC: "issued by"
  TBS ||--|| ASSET: "describes"
....

==== Credential holder’s verifiable credential

TO DO: Specify requirements for CH VC.

* Needs to have discoverable `assertionMethod`
* DID method recommendations/requirements?
* Alignment with ToIP verifiable identifier

==== Identity assertion’s verifiable credential

The _<<_c2pa_claim_generator,C2PA claim generator>>_ MUST generate the `tbs` data structure as described in *TO DO: Xref tbs once merged* and present that to the _<<_credential_holder,credential holder>>_ for signature.

The _<<_credential_holder,credential holder>>_ MUST issue a _new_ _<<_w3c_verifiable_credential,W3C verifiable credential>>_ which satisfies the following requirements:
...
* The `credentialSubject` field MUST contain a `tbs` entry, which MUST be the JSON serialization of the `tbs` data structure presented for signature. All bytestring entries in the `tbs` data structure (i.e. `hash` entries in hashlink URL data structures) MUST be converted to the corresponding Base-64 plain-text strings. (TO DO: Specify Base-64 encoding options.)
...
----
{
  ...
  "credentialSubject": {
    "tbs": {
      ...
      // TO DO: Add other members of `tbs` structure as they are defined.
    }
  },
