...
This specification describes a framework that allows a _<<_named_actor,named actor>>_ to attest to their relationship to a specific _<<C2PA asset>>_ produced by them or on their behalf.

This framework takes the form of a _<<C2PA assertion>>_ which we call the *<<_identity_assertion,identity assertion>>.* This assertion provides additional trust signals that can be used when interpreting the _<<C2PA Manifest>>_ in which it appears.

An _<<_identity_assertion_consumer,identity assertion consumer>>,_ upon finding that the *<<_identity_assertion,identity assertion>>* and the _<<C2PA Manifest>>_ in which it appears are valid and that the credential is from a source that it deems trustworthy, can then interpret this assertion as follows:

[quote,Signed by credential holder]
____
The _<<_named_actor,named actor>>_ described by this credential has performed a role (e.g. produced, edited, published) in the creation of the _<<C2PA asset>>_ described by this _<<C2PA Manifest>>._
____

This specification defines that framework and specific forms of credentials that can be used within the framework.
