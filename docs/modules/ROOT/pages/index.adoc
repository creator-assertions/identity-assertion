...
*Preliminary draft 03 December 2023* +
...
//? Should "content bindings" below be a link to the relevant section in the C2PA spec?
...
//? "data structure" is intentionally vague below?
//? Would it make sense to propose a non-normative sample of such a data structure for simpler reading. In "6.1 Overview", there is an attempt at that for "identity assertions", a better attempt at "6.2 Schema and example".
//? It's easier to read a JSON than to have one desribed to you.
// If you agree, a few other items in the Glossary might benefit from the same addition.
...
//? Should this be specific about what is being signed: the entire list once, not each assertion individually?
...
//? Outside of redundancy, is there any other reason?
The `assertions` entry of the identity assertion MUST NOT reference any assertion more than once.

//? Aren't these two statements contradictory? What am I missing?
...
//? In my opinion, the standard should not attempt to classify or otherwise rank identity claims. It's up to the content author and content consumer to determine what's reliable to them and what isn't.
//? That is to say, I'd keep a verified name VC and a verified Twitter account info under the same umbrella.
...
//? Should "Replace placeholder assertions with final identity assertions" be preceeded by a signature check on the C2PA claim generator side?
[mermaid,width=100%]
....
sequenceDiagram
...
//? Should there be validation that signatures match what was signed?
//? cca.identity.signature.invalid
=== Validation method
...
//? The subject of a DID could be a collective. As the spec states: "Anything can be the subject of a DID: person, group, organization, thing, or concept.". [https://www.w3.org/TR/did-core/#brief-architecture-overview-longdesc]
//? Our definition of `actor` is more limited: "a human or non-human (hardware or software) that is participating in the C2PA ecosystem."
...
//? This needs a broader discussion. Custodial wallets such as the one IMS will host can allow for all sorts of customizations but once we allow users to bring their own key, this will get more difficult (i.e. thumbnail).
...
//? I'm not sure what this means. Is the question should platforms displaying assets be able to verify the VP? If yes, then, in my opinion, they should be able to. Anyone should be able to validate signatures. But I suspect I'm misreading the note here.
