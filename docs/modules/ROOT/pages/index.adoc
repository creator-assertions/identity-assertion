...
*Preliminary draft 13 December 2023* +
...
This is intended to provide an additional trust signal over and above the signature provided by the _<<C2PA claim generator>>,_ as illustrated in the following diagram:
...
Each identity assertion contains hashlink references to one or more other _<<_c2pa_assertion,C2PA assertions>>_ and a signature from the _<<_credential_subject,credential subject>>_ over that list of hashlink references, thus providing a non-repudiable, tamper-evident binding between the _<<_credential_subject,credential subject>>_ and the list of assertions.
...
A temporary _<<C2PA assertion>>_ that is created during _<<C2PA claim>>_ generation which reserves space for the eventual content of identity assertion. A placeholder assertion MUST be used when the final file layout of the _<<C2PA asset>>_ is required for the _<<_content_binding,content binding>>_ assertion.
...
The content of `assertions` is a list of assertions which is presented to the _<<_credential_subject,credential subject>>_ for signature. This list of assertions MUST include a _<<_content_binding,content binding>>_ assertion as described in link:++https://c2pa.org/specifications/specifications/1.4/specs/C2PA_Specification.html#_hard_bindings++[Section 10.2, “Hard Bindings,”] of the C2PA technical specification.
...
An example in https://www.rfc-editor.org/rfc/rfc8949.html#name-diagnostic-notation[C`BO`R-Diag] is shown below:
...
Prior to requesting the credential holder with the list of assertions for signature, the assertions to be signed MUST themselves be created. The choice of which assertions to reference is not specified, except that the list of assertions MUST include the same _<<_content_binding,content binding>>_ assertion that is present in the _<<C2PA claim>>_ itself. This process is described in link:++https://c2pa.org/specifications/specifications/1.4/specs/C2PA_Specification.html#_creating_assertions++[Section 11.3.1, “Creating assertions,”] of the C2PA technical specification.
...
The list of assertions that will be referenced by the identity assertion MUST be presented to be signed by the _<<_credential_subject,credential subject>>_ corresponding to each identity assertion. This process is described in more detail in xref:_credentials_signatures_and_validation_methods[xrefstyle=full].
...
The validator MUST ensure that the `assertions` field contains at least one _<<_content_binding,content binding>>_ assertion as described in link:++https://c2pa.org/specifications/specifications/1.4/specs/C2PA_Specification.html#_hard_bindings++[Section 10.2, “Hard Bindings”] of the C2PA technical specification. The `cca.identity.content_binding_missing` error code SHALL be used to report a missing _<<_content_binding,content binding>>_ assertion.
...
|`cca.identity.content_binding_missing` | The identity assertion does not reference a _<<_content_binding,content binding>>_ assertion.
...
The _<<C2PA claim generator>>_ prepares the list of _<<_c2pa_assertion,C2PA assertions>>_ to be presented to the _<<_credential_subject,credential subject>>,_ which MUST in turn respond with a *verifiable presentation* as defined in link:++https://www.w3.org/TR/vc-data-model/#presentations-0++[Section 4.10, “Presentations,”] of the W3C verifiable credentials data model. Additional requirements for this presentation are stated in xref:_verifiable_presentation_requirements[xrefstyle=full].
...
NOTE: TO DO: Specify structure, including how list of assertions is expressed, for VP.
...
* The list of assertions in the verifiable presentation must match the assertions referenced in the `assertions` field of the identity assertion structure.
...
* The `proof` in the verifiable presentation indicated a valid signature for the public key cryptographic material in the DID document and the list of assertions.
