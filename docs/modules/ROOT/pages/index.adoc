...
*Preliminary draft 03 December 2023* _(with pending changes to W3C VC workflow)_ +
...
* https://www.w3.org/TR/did-core/[Decentralized identifiers (DIDs)] as defined by W3C, though support will be limited to specifically-chosen DID methods with wide adoption and independent verifiability.
...
The `assertions` entry of an identity assertion MAY reference any other identity assertion. A _<<C2PA claim generator>>_ MUST NOT allow any two identity assertions to refer to each other directly or indirectly.
...
IMPORTANT: TO DO: *Major discussion item:* I think there are reasonable cases for the content of the identity assertion to be a new Verifiable Credential and also for it to be a Verifiable Presentation. The VC path will resolve some of the redundant-chain-of-trust issues identified below, but may complicate the signing experience for wallet-based applications.
...
All _W3C decentralized identifiers_ that are stored in an identity assertion SHALL be generated using one of the _DID methods_ as described in this section. This section defines both:

* A list of _DID methods_ that are allowed for generating identifiers for new identity assertions as well as required for validating identifiers of existing identity assertions (the allowed list), and
* A list of _DID methods_ that are required to be supported for validating identifiers of existing identity assertions but are not allowed for generating identifiers for new identity assertions (the deprecated list).
...
* A list of _verification methods_ that are allowed when generating new identity assertions as well as required for validating public key material for existing identity assertions (the allowed list), and
* A list of _verification methods_ that are required to be supported for validating public key material for existing identity assertions but are not allowed for generating new identity assertions (the deprecated list).
