...
Version 1.x (adding W3C VC credentials with credential schema) *Draft 17 July 2024* · xref:_version_history[]
...
If the *<<_identity_assertion,identity assertion>> generator* chooses to include the `expected_partial_claim` field in `signer_payload`, it MUST be computed as follows:
...
If the *<<_identity_assertion,identity assertion>> generator* chooses to include the `expected_countersigners` field in `signer_payload`, it MUST be computed as follows:
...
* If the issuer is referenced on a trust list (a.k.a. _link:++https://trustoverip.github.io/ctwg-main-glossary/#term:trust-registry++[trust registry]_), do I trust the governance of that trust list? In other words, do I have confidence in the process by which this issuer was added to and has remained on the trust list?
...
Technical trust in a digital context is typically described using a _trust triangle_ as shown in the following diagram:
