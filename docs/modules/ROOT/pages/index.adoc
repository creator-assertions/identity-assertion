...
* String format
+
When specified as a string, the `issuer` MUST specify a valid URI that, when dereferenced, results in a document that can be used to verify the information from the credential, as specified in link:++https://www.w3.org/TR/vc-data-model-2.0/#issuer++[§4.7, “Issuer,”] of the Verifiable credentials data model.
+
.Example:
[source,json]
...
* Object format
+
When specified as an object, the `issuer` MUST include an `id` property. The `id` property MUST be a valid URI that, when dereferenced, results in a document that can be used to verify the information from the credential, as specified in link:++https://www.w3.org/TR/vc-data-model-2.0/#issuer++[§4.7, “Issuer,”] of the Verifiable credentials data model.
+
...
Validity::
The `validFrom` field MUST exist and it MUST express the date and time when the credential becomes valid, as specified in link:++https://www.w3.org/TR/vc-data-model-2.0/#validity-period++[§4.8, “Validity period,”] of the Verifiable credentials data model.
...
Person::
The `person` property MUST be an object that MAY be present. If present, `person` MUST contain information about the _<<_named_actor,named actor>>_ whose identity has been verified. The method of verification is beyond the scope of this specification. The `person` property MUST contain at least two child properties: `name` and `verifiedBy`.
+
.Example:
[source,json]
----
...
Person name::
The `person.name` property MUST be present and MUST not be empty. The `person.name` property defines the _<<_named_actor,named actor’s>>_ name as it was verified by one or multiple _<<Identity verifier,identity verifiers>>_.

[#vc-credentialsubject-person-verifiedBy]
Person verification details::
The `person.verifiedBy` property MUST be a non empty array and MUST be present. Each item of the array MUST contain details about the _<<_identity_verifier,identity verifier>>_ and the identity verification process. This specification mentions at least three properties that MAY be used to represent the person verification details: `id`, `name`, and `proof`.

[#vc-credentialsubject-person-verifiedBy-id]
Identity verifier ID:::
The `person.verifiedBy[?].id` MUST be present and MUST be a valid URI that, when dereferenced, MUST result in a proof of authenticity of the _<<_identity_verifier,identity verifier>>_.
...
Identity user name:::
The `identities[?].username` MUST be present and MUST NOT be empty. It represents the social account's user name.

[#vc-credentialsubject-identities-url]
Identity URL:::
The `identities[?].url` MUST be present and MUST be a valid URI. It represents an URL that can prove the existence of an account for the user name.

[#vc-credentialsubject-identities-provider]
Identity provider:::
The `identities[?].provider` MUST be present. It represents a string that identifies the social provider type. 
...
This identity representation presents the details of a crypto wallet account of the _<<_named_actor,named actor>>_. The methods for verifying crypto wallet accounts and their association with the _<<_named_actor,named actor>>_ are outside the scope of this specifiation. An identity by crypto wallet needs to contain at least the following child properties: `type`, `username`, `address`, `provider`, and `createdAt`.

Identity by crypto wallet type:::
In this case the `identities[?].type` property MUST be present and MUST be `cryptoWallet`.

Identity user name:::
The `identities[?].username` MUST be present and MUST NOT be empty. It represents the crypto wallet account’s user name.

Identity address:::
The `identities[?].address` MUST be present and MUST be a valid crypto wallet address. It represents an URL that can prove the existence of an account for the user name.

Identity provider:::
The `identities[?].provider` MUST be present. It represents a unique alphanumeric string that can be used to identity the user. _(Is it the user or the crypto wallet provider?)_
...
NOTE: TO DO: Eric’s detailed review SHOULD continue from here ...
...
The Identity assertion verifiable credential must adhere to a set of strict requirements as depicted in <<#_creator_identity_assertion, Creator identity assertion>>. All of there requirements are gathered in a https://www.w3.org/TR/vc-json-schema/[Verifiable Credentials JSON Schema] i.e. `https://creator-assertions.github.io/schemas/v1/creator-identity-assertion.json`. This JSON schema SHOULD be used to establish if the structure and contents of a verifiable credential conforms to all Identity assertion verifiable credential requirements.
