...
Version 1.x (adding W3C VC credentials with credential schema) *Draft 24 June 2024* · xref:_version_history[]

IMPORTANT: This specification differs from the link:https://creator-assertions.github.io/identity/1.0-draft/[1.0-draft version] primarily in the addition of xref:_w3c_verifiable_credential_2[xrefstyle=full].
...
signer-payload-map = {
  ...
  ? "expected_claim": $hash-map, ; hash of expected partial claim
  ? "expected_claim_generator": $hash-map, ; hash of expected claim signer credential
  ? "expected_countersigners": [1* $hash-map], ; array of expected countersigner credentials
}
...
==== Calculating the `expected_claim` value

If the *<<_identity_assertion,identity assertion>> generator* chooses to include the `expected_claim` value in `signer_payload`, it MUST be computed as follows:
...
The `expected_claim` value SHOULD contain a CBOR map with the following fields:
...
==== Calculating the `expected_claim_generator` value

If the *<<_identity_assertion,identity assertion>> generator* chooses to include the `expected_claim_generator` value in `signer_payload`, it MUST be computed as follows:
...
The resulting hash should be used as the `expected_claim_generator` value.

The `expected_claim_generator` value SHOULD contain a CBOR map with the following fields:

* `alg` describing the hash algorithm as described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_hashing_2++[Section 13.1, “Hashing,”] of the C2PA technical specification, and
* `hash` value containing the computed hash of the serialized certificate.

==== Calculating the `expected_countersigners` value

If the *<<_identity_assertion,identity assertion>> generator* chooses to include the `expected_countersigners` value in `signer_payload`, it MUST be computed as follows:

. For every _other_ *<<_identity_assertion,identity assertion>>* that will be present in the _<<C2PA Manifest>>:_
.. Identify the signing credential that will be used to sign that *<<_identity_assertion,identity assertion>>.*
...
.. Compute a hash of the serialized end-entity certificate using one of the algorithms documented in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_hashing_2++[Section 13.1, “Hashing,”] of the C2PA technical specification.
.. An entry in `expected_countersigners` SHOULD be added containing the hash from the previous step and the corresponding `alg` value for the selected hash algorithm.
. Once the signing credentials have been gathered, the `expected_countersigners` value SHOULD contain a CBOR array of CBOR maps with the following fields:
.. `alg` describing the hash algorithm as described in link:++https://c2pa.org/specifications/specifications/2.0/specs/C2PA_Specification.html#_hashing_2++[Section 13.1, “Hashing,”] of the C2PA technical specification, and
...
==== Validating the `expected_claim` value

If the *<<_identity_assertion,identity assertion>>* contains an `expected_claim` value in `signer_payload`, it MUST be validated as follows:
...
. If the *<<_identity_assertion,identity assertion>>* contains an `expected_countersigners` value in `signer_payload`, for each entry in the `expected_countersigners` array where an *<<_identity_assertion,identity assertion>>* can be found that matches the signing credentials used as described in xref:_serialization_for_credential_types[], replace the `hash` value for that *<<_identity_assertion,identity assertion>>* being validated with all zero-value (`0x00`) bytes.
...
. Compute a hash of the serialized _<<C2PA claim>>_ data structure using the hash algorithm described by `expected_claim.alg`.

The resulting hash MUST exactly match the value present in `expected_claim.hash`. The `cawg.identity.expected_claim.mismatch` error code SHALL be used to report assertions that contain other values.

==== Validating the `expected_claim_generator` value

If the *<<_identity_assertion,identity assertion>>* contains an `expected_claim_generator` value in `signer_payload`, it MUST be validated as follows:
...
|`cawg.identity.expected_claim.mismatch` | The `signer_payload.expected_claim` field did not match the _<<C2PA claim>>_ data structure.
...
In some use cases, an _<<_actor,actor>>_ in the system may wish to generate a _<<W3C verifiable credential>>_ that describes a _<<C2PA asset>>._ This credential will include the information  contained in the `signer_payload` structure to ensure that it is bound to the specific _<<C2PA asset>>._ It MAY also include information such as:

* The identity of a _<<_named_actor,named actor>>_ who wishes to document their relationship to the asset
* Social media or other accounts that are related to the asset

The `signer_payload.sig_type` value for such an assertion MUST be `cawg.w3c.vc`.

The issuer is responsible for gathering information about the _<<_named_actor,named actor>>_ and the _<<C2PA asset>>_ and generating a new _<<W3C verifiable credential>>_ that describes the relationship between the two.
...
[#_creator_identity_assertion]
==== Creator identity assertion

A *creator identity assertion* is a _<<_w3c_verifiable_credential,W3C verifiable credential>>_ that binds the identity of an _<<_named_actor,named actor>>_ with their _<<C2PA asset>>_ contribution. A *creator identity assertion* MUST satisfy the requirements in the subsections that follow;

===== W3C verifiable credentials version compatibility
...
===== W3C verifiable credentials properties

[#vc-property-context]
Context:: The `@context` property MUST be present and MUST contain at least the following two entries:
* `https://www.w3.org/ns/credentials/v2`
* `https://creator-assertions.github.io/tbd/tbd`

NOTE: TO DO: Transition the creator identity assertion context to the recently-acquired domain cawg.io once it is provisioned and ready to use.

[#vc-property-type]
Type:: The `type` property MUST be present and MUST contain at least the following two entries:
** `VerifiableCredential`
** `CreatorIdentityAssertionCredential`

[#vc-property-issuer]
Issuer:: The `issuer` property MUST be present and MAY have one of two representations:
* String format
+
When specified as a string, the `issuer` MUST specify a valid URI that, when dereferenced, results in a document that can be used to verify the information from the credential, as specified in link:++https://www.w3.org/TR/vc-data-model-2.0/#issuer++[Section 4.7, “Issuer,”] of the Verifiable credentials data model.
+
.Example:
[source,json]
...
* Object format
+
When specified as an object, the `issuer` MUST include an `id` property. The `id` property MUST be a valid URI that, when dereferenced, results in a document that can be used to verify the information from the credential, as specified in link:++https://www.w3.org/TR/vc-data-model-2.0/#issuer++[Section 4.7, “Issuer,”] of the Verifiable credentials data model.
+
Additional properties MAY be included within the object to provide further details about the issuer, including a non-empty `name` property.
+
.Example:
[source,json]
----
"issuer": {
  ...
  "name": "Example issuer",
  "contact": "https://example.com/issuer"
}
---- 
...
===== Creator identity properties

The `credentialSubject` property MUST contain both the identity of _<<_named_actor,named actor>>_ and the _<<C2PA asset>>_ as described by the `signer_payload`. These can be represented by the following properties:

[#vc-credentialsubject-id]
ID::
The `id` MUST be present and MUST represent an URI. It is RECOMMENDED that the URI be one which, if dereferenced, results in a globally unambiguous status of existence of the _<<_named_actor,named actor>>_. For example, dereferencing a `did:web` might return a `204 No Content` HTTP status code in case the user exists and no further information is available or `404 Not Found` HTTP status code in case the user does not exist.

[#vc-credentialsubject-namedactor]
Named Actor::
The `namedActor` property MUST be an array that MAY be present. If present, every item of `namedActor` MUST contain information about the _<<_named_actor,named actor>>_ whose identity has been verified by a certain _<<Identity verifier,identity verifiers>>_. The method of verification is beyond the scope of this specification. Each item of `namedActor` property MUST contain at least two child properties: `name` and `verifiedBy`.
+
.Example:
[source,json]
----
...
---- 
[#vc-credentialsubject-namedactor-name]
Named Actor name::
The `namedActor[?].name` property MUST be present and MUST not be empty. The `namedActor[?].name` property defines the _<<_named_actor,named actor’s>>_ name as it was verified by the _<<Identity verifier,identity verifiers>>_.

[#vc-credentialsubject-namedactor-verifiedBy]
Named Actor verification details::
The `namedActor[?].verifiedBy` property MUST be an object and MUST be present. The `namedActor[?].verifiedBy` property MUST contain details about the _<<_identity_verifier,identity verifier>>_ and the identity verification process. This specification mentions at least three properties that MAY be used to represent the _<<_named_actor,named actor’s>>_ verification details: `id`, `name`, and `proof`.

[#vc-credentialsubject-namedactor-verifiedBy-id]
Identity verifier ID:::
The `namedActor[?].verifiedBy.id` MUST be present and MUST be a valid URI that, when dereferenced, MUST result in a proof of authenticity of the _<<_identity_verifier,identity verifier>>_. This proof of authenticity of the _<<_identity_verifier,identity verifier>>_ must not be confused with the proof of verification of the _<<_named_actor,named actor>>_.

[#vc-credentialsubject-namedactor-verifiedBy-name]
Identity verifier name:::
The `namedActor[?].verifiedBy.name` MUST be present and MUST not be empty. The `namedActor[?].verifiedBy.name` property is the name of the _<<_identity_verifier,identity verifier>>_.

[#vc-credentialsubject-namedactor-verifiedBy-proof]
Identity verification proof:::
The `namedActor[?].verifiedBy.proof` is an optional field. If present, it MUST be a valid URI that, when dereferenced, MUST result in the proof of authenticity of the  _<<_named_actor,named actor>>_, as per the verification process of the _<<_identity_verifier,identity verifier>>_. The content as well as the process are outside the scope of this specification.

[#vc-credentialsubject-affiliations]
Affiliations::
The `affiliations` property is an optional field. If present, it MUST be an array in which each item MUST contain information about a _<<_named_actor,named actor's>>_ affiliation. The process of proving the validity of these affiliations is outside the scope of this specification. This specification mentions at least three properties that MAY be used to represent the _<<_named_actor,named actor’s>>_ affiliation details: `id`, `name`, and `proof`.
+
.Example:
[source,json]
----
...
----
[#vc-credentialsubject-affiliation-id]
Affiliation ID:::
The `affiliations[?].id` MUST be present and MUST be a valid URI that, when dereferenced, MUST result in a proof of authenticity of the _<<_named_actor,named actor's>>_ affiliated entity.

[#vc-credentialsubject-affiliation-name]
Affiliation name:::
The `affiliations[?].name` MUST be present and MUST not be empty. The `affiliations[?].name` property is the name of the  _<<_named_actor,named actor's>>_ affiliated entity.

[#vc-credentialsubject-affiliation-proof]
Affiliation proof:::
The `affiliations[?].proof` is an optional field. If present, it MUST be a valid URI that, when dereferenced, MUST result in the proof of authenticity of the  _<<_named_actor,named actor's>>_ affiliation validity, i.e., the _<<_named_actor,named actor>> is affiliated indeed with this organization. The content as well as the process of validatig an affiliation relation are outside the scope of this specification.

[#vc-credentialsubject-identities]
Identities::
The `identities` property is an optional field. If present, it MUST be an array containing information about the accounts that the _<<_named_actor,named actor>>_ requested to be bound with the _<<C2PA asset>>_ being described.
+
.Example:
[source,json]
----
  ...
  "identities": [
    {
      ...
      "createdAt": "2024-05-27T08:40:39.569856Z"      
    }
  ]
  ...
}
----
This specification mentions two identity types that MAY be used, but this specification is not limitted to these two:
* `socialProvider`
* `cryptoWallet`

[#vc-credentialsubject-identities-socialProvider]
Identity by social provider::
This identity representation presents the details of a social account of the _<<_named_actor,named actor>>_. The method of social account verification is outside the scope of this specifiation. An identity by social provider MUST contain at least the following four child properties: `type`, `uri`, `provider`, and `createdAt`. Additionally, an identity by social provider MAY contain an `username` property.

[#vc-credentialsubject-identities-type]
Identity by social provider type:::
In this case the `identities[?].type` property MUST be present and MUST be `socialProvider`.

[#vc-credentialsubject-identities-username]
Identity user name:::
The `identities[?].username` MAY be present and, if present, MUST NOT be empty. It represents the social account's user name.

[#vc-credentialsubject-identities-uri]
Identity URI:::
The `identities[?].uri` MUST be present and MUST be a valid URI. It represents an URI that can prove the existence of an account for the user name.

[#vc-credentialsubject-identities-provider]
Identity provider:::
The `identities[?].provider` MUST be a non empty object. The `identities[?].provider` object contains information about the social provider that is associated with this identity.  A social provider MUST contain at least the following two child properties: `id` and `name`.

[#vc-credentialsubject-identities-provider-id]
Identity provider ID:::
The `identities[?].provider.id` MUST be present and MUST be a valid URI that, when dereferenced, MUST result in a proof existence of the social provider.

[#vc-credentialsubject-identities-provider-name]
Identity provider name:::
The `identities[?].provider.name` MUST be present and MUST be a non emtpy string that represents the name of the social provider.

Identity creation date:::
The `identities[?].createdAt` MUST be present and MUST be a valid date-time as specified by https://datatracker.ietf.org/doc/html/rfc3339[RFC 3339]. It represents the date and time when this social account was associated with the _<<_named_actor,named actor>>_. 

Identity by crypto wallet:::
This identity representation presents the details of a crypto wallet account of the _<<_named_actor,named actor>>_. The methods for verifying crypto wallet accounts and their association with the _<<_named_actor,named actor>>_ are outside the scope of this specifiation. An identity by crypto wallet needs to contain at least the following four child properties: `type`, `address`, `provider`, and `createdAt`.

Identity by crypto wallet type:::
In this case the `identities[?].type` property MUST be present and MUST be `cryptoWallet`.

Identity by crypto wallet address:::
The `identities[?].address` MUST be present and MUST be a valid crypto wallet address. It represents a unique alphanumeric string that can be used to identity the user's crypto wallet.

[#vc-credentialsubject-identities-cw-provider]
Identity provider:::
The `identities[?].provider` MUST be a non empty object. The `identities[?].provider` object contains information about the crypto wallet provider that is associated with this identity.  A crypto wallet MUST contain at least the following two child properties: `id` and `name`.

[#vc-credentialsubject-identities-cw-provider-id]
Identity provider ID:::
The `identities[?].provider.id` MUST be present and MUST be a valid URI that, when dereferenced, MUST result in a proof existence of the cryto wallet provider.

[#vc-credentialsubject-identities-cw-provider-name]
Identity provider name:::
The `identities[?].provider.name` MUST be present and MUST be a non emtpy string that represents the name of the crypto wallet provider.

Identity creation date:::
The `identities[?].createdAt` MUST be present and MUST be a valid date-time as specified by https://datatracker.ietf.org/doc/html/rfc3339[RFC 3339]. It represents the date and time when this crypto wallet account was associated with the _<<_named_actor,named actor>>_. 

IMPORTANT: At least one of `namedActor` property and `identities` property MUST be present, as they represent _<<_named_actor,named actor>>_ identity to be bound to with the _<<C2PA asset>>_ being described. Having both missing makes the credential invalid as there is no identity to be bound to the _<<C2PA asset>>._

Credential C2PA asset::
The `credentialSubject` field MUST contain a `boundTo` entry, which MUST be the JSON serialization of the `signer_payload` data structure presented for signature. All bytestring entries in the `signer_payload` data structure (i.e. hash entries in hashlink URL data structures) MUST be converted to the corresponding Base-64 plain-text strings. (TO DO: Specify Base-64 encoding options.)
+
.Example:
[source,json]
----
...
==== Identity assertion verifiable credential example

An example of the *<<_identity_assertion,identity assertion>>* verifiable credential is given below:
    ...
    "namedActor": [
      {
        "name": "John Doe",
        "verifiedBy": {
          "id": "https://verifier-3732.com",
          "name": "Verifier 3732",
          "proof": "https://verifier-3732.com/proofs/3732"
        }
      }
    ],
    "affiliations": [
...
The inclusion of the `creator-identity-assertion.json` schema in the verifiable credential `credentialSchema` is OPTIONAL. However, it is highly recommended to include it as it enforces this specification’s constraints.

==== Recommendations for verifiers

The usage of the data verification schemas from a verifiable credential's `credentialSchema` is OPTIONAL. However, it is recommended that verifiers use the `credentialSchema` to validate the structure and data integrity of the verifiable credential. By referencing the schemas specified in the `credentialSchema` field, verifiers can ensure that the credential data adheres to the expected format and rules.

NOTE: `creator-identity-assertion.json` makes use of the `format` JSON Schema keyword. If used, the JSON Schema processor must enable the `format` usage.
...
NOTE: TO DO (link:https://github.com/creator-assertions/identity-assertion/issues/117[issue #117]): Discuss and add guidance.
...
* `expected_claim`, which describes the content of the expected _<<C2PA claim>>_ in which the *<<_identity_assertion,identity assertion>>* appears.
