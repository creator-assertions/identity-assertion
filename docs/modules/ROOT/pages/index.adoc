...
expected-countersigner-map = {
  "expected_signer_payload": $signer-payload-map ; signer_payload value from other identity assertion
...
.. Calculate the `signer_payload` value for that *<<_identity_assertion,identity assertion>>.*
...
.. `expected_signer_payload` _(required)_ containing the expected `signer_payload`.
