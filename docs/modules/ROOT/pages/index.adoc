...
Version 1.x (adding identity claim aggregation) *Draft 03 September 2024* · xref:_version_history[]

IMPORTANT: This specification differs from the link:https://creator-assertions.github.io/identity/1.0-draft/[1.0-draft version] primarily in the addition of xref:_identity_claim_aggregation[xrefstyle=full].
...
In that case, they may arrange with an _<<_identity_claim_aggregator,identity claim aggregator>>_ to collect identity attestation claims from various _<<_identity_provider,identity providers>>_ (social media sites, ID verification vendors, etc.) and replay those identity attestation claims on their behalf to describe their role in producing a specific _<<C2PA asset>>._

The trust model in this scenario is as described in xref:_named_actor_without_signature_authority[xrefstyle=full].

The _<<_identity_claim_aggregator,identity claim aggregator>>_ will produce a specific type of _<<_W3C verifiable credential,W3C verifiable credential>>_ called a “creator identity assertion credential” that binds the identity attestation claims to the _<<C2PA asset>>._ This credential, once signed with the _<<_identity_claim_aggregator,identity claim aggregator’s>>_ signature, is the `signature` value for the *<<_identity_assertion,identity assertion>>.*

NOTE: TO DISCUSS: We use the terms “identity claim aggregation” and “creator identity assertion credential” to mean essentially the same thing. Can we unify on one term? If so, which one?

The `signer_payload.sig_type` value for such an assertion MUST be `cawg.identity_claim_aggregation`.
